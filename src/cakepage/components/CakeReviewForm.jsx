...
function CakeReviewForm({
  ...
  const theme =
    designDetails.theme === 'Other'
      ? designDetails.otherTheme || 'Other'
      : designDetails.theme || emptyCustomerText
  ...
  const recipientName = [customerInfo.recipientLastName, customerInfo.recipientFirstName]
    .filter(Boolean)
    .join(', ')
  const customerName = [customerInfo.customerLastName, customerInfo.customerFirstName]
    .filter(Boolean)
    .join(', ')
  const displayedRecipientName = customerInfo.deliverDifferentRecipient
    ? recipientName
    : customerName
  const displayedRecipientContact = customerInfo.deliverDifferentRecipient
    ? customerInfo.recipientContact
    : customerInfo.contactNumber
  const isDelivery = customerInfo.fulfillment === 'delivery'
  const customerItems = [
    { label: 'Customer Name', value: customerName || emptyCustomerText },
    ...
    ...(isDelivery
      ? [
          { label: 'Recipient Name', value: displayedRecipientName || fallbackText },
          {
            label: 'Recipient Contact Number',
            value: displayedRecipientContact || fallbackText,
          },
  ...
  return (
    <section className="cake-base-form cake-review-form" aria-labelledby="cake-review-title">
      <div className="cake-form-heading">
        <h2 id="cake-review-title">Review Your Order</h2>
      ...
      <div className="cake-review-content">
        <ReviewSection
          ...
          items={[
            ...
            { label: 'Size', value: selections.size ? `${selections.size}"` : emptyCustomerText },
            { label: 'Layers', value: layerLabels[selections.layers] ?? emptyCustomerText },
          ]}
        />
        ...
        <ReviewSection
          ...
          items={[
            ...
            {
              label: 'Cake Message',
              value: designDetails.message || fallbackText,
              isEmpty: !designDetails.message,
            },
            {
              label: 'Special Instructions',
              value: designDetails.instructions || fallbackText,
              isEmpty: !designDetails.instructions,
            },
          ]}
        />
